...
typedef struct Indictator
{
    float sma;
    float padding1; // Can be used for future indictators inclusion like DI+/DI-/ADX and so forth
    float padding2;
    float padding3;
} Indictator;
...
void InitializeBuffers(ComputeApplication this)
{
    ...
    bufferCreateInfo = (VkBufferCreateInfo){
        .sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
        .size = sizeof(Indictator) * this->inputDataElementsCount,
        .usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        .sharingMode = VK_SHARING_MODE_EXCLUSIVE,
    };
...
void InitializeDescriptorSetLayout(ComputeApplication this)
{
...
void InitializeDescriptorSets(ComputeApplication this)
{
    ...
    descriptorBufferInfo[1].range = sizeof(Indictator) * this->inputDataElementsCount;
...
uint32_t* readFile(const char *filename, uint32_t* outlength)
{
...
void InitializeComputePipelines(ComputeApplication this)
{
    uint32_t filelength;
    uint32_t *code = readFile("smaPass1.spv", &filelength);
    ...
    code = readFile("smaPass2.spv", &filelength);
...
void InitializeCommandBuffers(ComputeApplication this)
{
    ...
    bufferCopy = (VkBufferCopy){
        .size = this->inputDataElementsCount * sizeof(Indictator),
        .dstOffset = 0,
        .srcOffset = 0
    };
...
void ExecuteComputeShaders(ComputeApplication this)
{
...
void LoadSampleFile(ComputeApplication this)
{
...
void CopySampleDataIntoInputBuffer(ComputeApplication this)
{
...
void PrintAllResults(ComputeApplication this)
{
    Candlestick* mappedMemory = NULL;
    Indictator* outputMem = NULL;
...
int main()
{
    ComputeApplication app = (ComputeApplication)calloc(1, sizeof(struct ComputeApplication));
    run(app);
    return EXIT_SUCCESS;
}
